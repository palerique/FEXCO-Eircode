(function() {
  ...
  QUnit.test('isEqual', function(assert) {
    ...
    assert.notOk(_.isEqual(String('Curly'), String('Larry')), 'String objects with different primitive values are not equal');
    assert.notOk(_.isEqual(String('Curly'), {toString: function(){ return 'Curly'; }}), 'String objects and objects with a custom `toString` method are not equal');
    ...
    assert.notOk(_.isEqual(Number(0), -0), '`new Number(0)` and `-0` are not equal');
    assert.notOk(_.isEqual(0, Number(-0)), 'Commutative equality is implemented for `new Number(0)` and `-0`');

    assert.notOk(_.isEqual(Number(75), Number(63)), 'Number objects with different primitive values are not equal');
    assert.notOk(_.isEqual(Number(63), {valueOf: function(){ return 63; }}), 'Number objects and objects with a `valueOf` method are not equal');
    ...
    assert.notOk(_.isEqual(Number(79), NaN), 'A number object is not equal to `NaN`');
    ...
    assert.notOk(_.isEqual(Boolean(true), Boolean), 'Boolean objects with different primitive values are not equal');

    // Common type coercions.
    assert.notOk(_.isEqual(Boolean(false), true), '`new Boolean(false)` is not equal to `true`');
    assert.notOk(_.isEqual('75', 75), 'String and number primitives with like values are not equal');
    assert.notOk(_.isEqual(Number(63), String(63)), 'String and number objects with like values are not equal');
    ...
    assert.notOk(_.isEqual(Boolean(false), Number(0)), 'Boolean and number objects with like values are not equal');
    assert.notOk(_.isEqual(false, String('')), 'Boolean primitives and string objects with like values are not equal');
  ...
  QUnit.test('isNaN', function(assert) {
    ...
    assert.notOk(_.isNaN(Number(0)), 'wrapped 0 is not NaN');
